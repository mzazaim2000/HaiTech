...
// CreatePart
      ...
      if (this.readyState == 4 && this.status == 200) {
        ...
        Swal.fire({
          ...
          html:
            ...
            '<input id="lname" class="swal2-input" placeholder="Last" value="'+user['sname']+'">' +
            '<input id="email" class="swal2-input" placeholder="Username" value="'+user['email']+'">' +
            '<input id="contact" class="swal2-input" placeholder="Email" value="'+user['contact']+'">',
          focusConfirm: false,
          preConfirm: () => {
            userEdit();
          }
  ...
  function userEdit() {
    ...
    xhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
    xhttp.send(JSON.stringify({ 
      "id": id, "fname": fname, "sname": sname, "email": email, "contact": contact, 
    }));
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        const objects = JSON.parse(this.responseText);
